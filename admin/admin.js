...
var WIZARD_IFRAME_BUILD='20260530s';
...
function shouldShowBackExtraControls(backMode){
  var mode=backMode||((((S.d||{}).meta||{}).backMode)||'mirror');
...
function syncBackSurfaceUiState(){
  var activeSurface=(STYLE_BACK_EDIT_SURFACE==='back')?'back':'front';
...
function runBackPreviewTransition(opts){
  ...
  function revealExtraControlsAfterDelay(){
    if(!delayExtraReveal)return;
  ...
  if(roundTrip&&fromFlipped&&toFlipped){
    syncBackModeUiState();
    syncBackSurfaceUiState();
    syncBackExtraControlsUiState(false);
    setPreviewCardFlipState(false);
    BACK_EDITOR_REFRESH_TIMER=setTimeout(function(){
      BACK_EDITOR_REFRESH_TIMER=0;
      ...
      requestAnimationFrame(function(){
        requestAnimationFrame(function(){
          setPreviewCardFlipState(true);
          revealExtraControlsAfterDelay();
        });
      });
    },BACK_PREVIEW_REBUILD_DELAY);
    return;
...
function isEditingBackDesign(){
  return S.clTab==='opmaken'&&S.opmPane==='vragen'&&isCustomBackMode((S.d||{}).meta||{})&&STYLE_BACK_EDIT_SURFACE==='back';
}
...
function buildSharedCardEditor(opts){
  ...
  var editingBackSurface=(backMode==='blank'&&STYLE_BACK_EDIT_SURFACE==='back');
  ...
  var showPreviewBackToolbarExtra=shouldShowBackExtraControls(backMode);
  var previewBackToolbarExtraHtml=
    '<div class="previewEditSurfaceStack'+(showPreviewBackToolbarExtra?' is-visible':' is-hidden')+'"'+(showPreviewBackToolbarExtra?'':' aria-hidden="true"')+'>'+
      '<div class="previewEditSurfacePill previewEditSurfacePill-surface">'+
        '<button class="previewEditSurfaceBtn'+(!editingBackSurface?' sel':'')+'" data-back-edit-surface="front" type="button" title="Voorkant ontwerpen" onclick="setBackEditSurface(\'front\')"'+(showPreviewBackToolbarExtra?'':' tabindex="-1"')+'>Voor</button>'+
        '<button class="previewEditSurfaceBtn'+(editingBackSurface?' sel':'')+'" data-back-edit-surface="back" type="button" title="Achterkant ontwerpen" onclick="setBackEditSurface(\'back\')"'+(showPreviewBackToolbarExtra?'':' tabindex="-1"')+'>Achter</button>'+
      '</div>'+
    ...
    ?(canUseSharedPreviewShell
      ...
      :'<div class="stijlCanvasBackbar">'+
          ...
          '<div class="stijlCanvasBackbarGroup">'+
            '<button class="stijlBackModeBtn'+(backMode==='mirror'?' sel':'')+'" data-back-mode="mirror" type="button" onclick="setBackMode(\'mirror\')" title="Zelfde ontwerp">Zelfde</button>'+
            '<button class="stijlBackModeBtn'+(backMode==='reflect'?' sel':'')+'" data-back-mode="reflect" type="button" onclick="setBackMode(\'reflect\')" title="Zelfde ontwerp gespiegeld">Gespiegeld</button>'+
            '<button class="stijlBackModeBtn'+(backMode==='blank'?' sel':'')+'" data-back-mode="blank" type="button" onclick="setBackMode(\'blank\')" title="Eigen achterkant">Eigen</button>'+
          '</div>'+
