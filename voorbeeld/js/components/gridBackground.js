...
function drawCustomLightBlobs(ctx, info, blobs, viewBox, opacityScale){
  ...
  if(typeof window.Path2D !== 'function') return false;
...
function buildAssets(opts){
  ...
  for(var i=0;i<files.length;i++){
    (function(file){
      promises.push(
        window.PK.getText(cardBase + file).then(function(txt){
          return {
...
export const gridBackground = {
  render: function(opts){
    var canvas = document.getElementById('indexBg');
    if(!canvas) return;
