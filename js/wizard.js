...
const PREVIEW_MODE_SWAP_DURATION = 380;
const PREVIEW_MODE_SWAP_DELAY = 170;
...
let previewModeSwapTimer = 0;
let previewModeTurnTimer = 0;
  ...
  return (
    '<div class="stijlCanvasTopbar wizardPreviewTopbar">' +
      ...
      '<div class="previewTopPill previewNavPill">' +
        '<button class="stijlCanvasNightBtn" type="button" onclick="window.PK.wizardPreview.navigate(-1);return false;" title="Vorige">' +
          previewNavArrowIconHtml('back') +
        '</button>' +
        '<button class="stijlCanvasNightBtn previewNavPosBtn" type="button" onclick="window.PK.wizardPreview.navigateHome();return false;" title="Ga naar cover">' +
          '<span class="previewNavPosText">' + esc(navLabel) + '</span>' +
        '</button>' +
        '<button class="stijlCanvasNightBtn" type="button" onclick="window.PK.wizardPreview.navigate(1);return false;" title="Volgende">' +
...
function clearWizardPreviewModeTimers() {
  if (previewModeSwapTimer) {
    clearTimeout(previewModeSwapTimer);
    previewModeSwapTimer = 0;
  }
  if (previewModeTurnTimer) {
    clearTimeout(previewModeTurnTimer);
    previewModeTurnTimer = 0;
  }
}
...
function startWizardPreviewBackSwapTurn() {
  var els = wizardPreviewElements();
  ...
  previewModeTurnTimer = setTimeout(function(){
    previewModeTurnTimer = 0;
    var nextEls = wizardPreviewElements();
    if (nextEls && nextEls.faceInner) nextEls.faceInner.classList.remove('back-swap-turn');
  }, PREVIEW_MODE_SWAP_DURATION + 40);
}
...
function setWizardPreviewBackMode(mode) {
  ...
  if (prevMode === nextMode && wasFlipped) return;
  state.wizardDraft.preview = state.wizardDraft.preview || {};
  ...
  state.previewFlipped = true;
  startWizardPreviewBackSwapTurn();
  previewModeSwapTimer = setTimeout(function(){
    previewModeSwapTimer = 0;
    replaceWizardPreviewBackFaceDom();
    applyWizardPreviewUiState();
  }, PREVIEW_MODE_SWAP_DELAY);
  applyWizardPreviewUiState();
}
