        ...
        '<div class="iconLibrary wizardDesignIconLibrary">' +
          ...
          '<div class="iconGridMini wizardDesignIconGrid wizardDesignIconQuickRow">' +
            quickIconItems.map(function(icon){
              var selected = String(state.wizardDraft.design.iconPreset || '').trim() === icon.id;
              return (
...
function renderPreviewPanel() {
  ...
  var previewShellHtml = sharedPreviewShell
    ...
    : (
      '<div class="stijlPreviewCol stijlCanvasCenter">' +
        '<div class="stijlCanvasStage" style="' + esc(metrics.stageStyle) + '">' +
          '<div class="stijlCanvasCardWrap">' +
            '<div class="' + esc(previewWindowClassName) + '" data-wizard-preview="1" style="' + esc(metrics.shellStyle) + '">' +
...
function renderPreviewControls(preview) {
  ...
  var canJumpCover = !!(navState.allowCover && navState.current && navState.current.target !== 'cover');
  var backAttrText = navState.hasPrev ? ' data-preview-nav="-1"' : ' data-preview-nav-disabled="1" aria-disabled="true" tabindex="-1"';
  var forwardAttrText = navState.hasNext ? ' data-preview-nav="1"' : ' data-preview-nav-disabled="1" aria-disabled="true" tabindex="-1"';
  var centerClassName = canJumpCover ? '' : 'previewNavPosStatic';
  var centerAttrText = canJumpCover
    ? ' data-preview-nav-home="1"'
  ...
  if (sharedPreviewShell && typeof sharedPreviewShell.buildTopbarHtml === 'function') {
    return sharedPreviewShell.buildTopbarHtml({
      ...
      nav: {
        backAttrText: navState.hasPrev ? ' onclick="window.PK.wizardPreview.navigate(-1);return false;"' : backAttrText,
        centerLabel: navLabel,
        centerClassName: centerClassName,
        centerTitle: canJumpCover ? 'Ga naar cover' : '',
        centerAttrText: canJumpCover ? ' onclick="window.PK.wizardPreview.navigateHome();return false;"' : centerAttrText,
        forwardAttrText: navState.hasNext ? ' onclick="window.PK.wizardPreview.navigate(1);return false;"' : forwardAttrText
      },
      actions: {
  ...
  return (
    '<div class="stijlCanvasTopbar wizardPreviewTopbar">' +
      '<div class="cvZoomControl wizardPreviewZoomControl">' +
        '<button class="cvZoomBtn" type="button" onclick="window.PK.wizardPreview.stepZoom(-' + PREVIEW_ZOOM_STEP + ');return false;" title="Uitzoomen">−</button>' +
        '<button class="cvZoomPct" type="button" onclick="window.PK.wizardPreview.resetZoom();return false;" title="Telefoon-formaat (' + PREVIEW_DEFAULT_ZOOM + '%)">' + esc(String(state.previewZoom || PREVIEW_DEFAULT_ZOOM)) + '%</button>' +
        '<button class="cvZoomBtn" type="button" onclick="window.PK.wizardPreview.stepZoom(' + PREVIEW_ZOOM_STEP + ');return false;" title="Inzoomen">+</button>' +
      '</div>' +
      '<div class="previewTopPill previewNavPill">' +
        '<button class="stijlCanvasNightBtn" type="button"' + (navState.hasPrev ? ' onclick="window.PK.wizardPreview.navigate(-1);return false;"' : backAttrText) + ' title="Vorige">' +
          previewNavArrowIconHtml('back') +
        '</button>' +
        '<button class="stijlCanvasNightBtn previewNavPosBtn' + (centerClassName ? ' ' + centerClassName : '') + '" type="button"' + (canJumpCover ? ' onclick="window.PK.wizardPreview.navigateHome();return false;"' : centerAttrText) + (canJumpCover ? ' title="Ga naar cover"' : '') + '>' +
          '<span class="previewNavPosText">' + esc(navLabel) + '</span>' +
        '</button>' +
        '<button class="stijlCanvasNightBtn" type="button"' + (navState.hasNext ? ' onclick="window.PK.wizardPreview.navigate(1);return false;"' : forwardAttrText) + ' title="Volgende">' +
      ...
      '<div class="stijlCanvasTopbarRight">' +
        '<div class="previewTopPill wizardPreviewActionPill">' +
          (doubleSided
            ? (
              '<button class="stijlCanvasFlipBtn' + (state.previewFlipped ? ' sel' : '') + '" type="button" data-preview-flip="1" onclick="window.PK.wizardPreview.toggleFlip();return false;" aria-label="Kaart omdraaien" title="Kaart omdraaien">' +
                '<span class="flipGlyph" aria-hidden="true">↻</span>' +
              '</button>' +
              '<div class="previewTopPillSep" aria-hidden="true"></div>'
            )
            : '') +
          '<button class="stijlCanvasGridBtn' + (gridMode ? ' sel' : '') + '" type="button" data-preview-grid-toggle="1" onclick="window.PK.wizardPreview.toggleGrid();return false;" aria-label="' + (gridMode ? 'Raster uitzetten' : 'Raster tonen') + '" title="' + (gridMode ? 'Raster uitzetten' : 'Raster tonen') + '">' +
            previewGridToggleIconHtml() +
          '</button>' +
          '<button class="stijlCanvasNightBtn' + (nightMode ? ' sel' : '') + '" type="button" data-preview-night-toggle="1" onclick="window.PK.wizardPreview.toggleNight();return false;" aria-label="' + (nightMode ? 'Nachtmodus uitzetten' : 'Nachtmodus aanzetten') + '" title="' + (nightMode ? 'Nachtmodus uitzetten' : 'Nachtmodus aanzetten') + '">' +
  ...
  root.querySelectorAll('[data-nav="back"]').forEach(function(backButton){
    backButton.addEventListener('click', function(){
      if (isDesignStep()) {
        ...
        if (designIndex > 0) {
          var previousSubstep = DESIGN_SUBSTEPS[designIndex - 1].id;
          setDesignSubstep(previousSubstep);
          commitWizardChange(false);
          return;
        }
      }
...
async function handleNext() {
  ...
  if (isDesignStep()) {
    ...
    if (designIndex < DESIGN_SUBSTEPS.length - 1) {
      var nextSubstep = DESIGN_SUBSTEPS[designIndex + 1].id;
      setDesignSubstep(nextSubstep);
      commitWizardChange(false);
      return;
    }
  }
  ...
  if (stepAllowsCoverPreview(stepId)) {
    if (previewTarget === 'theme') {
      return {
        bundle: bundle,
        label: 'Thema kaart',
        navLabel: activeRecord.label || 'Thema kaart',
  ...
  return (
    '<div class="stijlCanvasBackbar">' +
      ...
      '<div class="stijlCanvasBackbarGroup">' +
        '<button class="stijlBackModeBtn' + (backMode === 'mirror' ? ' sel' : '') + '" type="button" data-preview-back-mode="mirror" onclick="window.PK.wizardPreview.setBackMode(\'mirror\');return false;" title="Zelfde ontwerp">Zelfde</button>' +
        '<button class="stijlBackModeBtn' + (backMode === 'reflect' ? ' sel' : '') + '" type="button" data-preview-back-mode="reflect" onclick="window.PK.wizardPreview.setBackMode(\'reflect\');return false;" title="Zelfde ontwerp gespiegeld">Gespiegeld</button>' +
        '<button class="stijlBackModeBtn' + (backMode === 'blank' ? ' sel' : '') + '" type="button" data-preview-back-mode="blank" onclick="window.PK.wizardPreview.setBackMode(\'blank\');return false;" title="Eigen achterkant">Eigen</button>' +
      '</div>' +
    '</div>'
  );
}
...
function syncWizardPreviewLayout() {
  ...
  var sharedLayoutSync = window.PK && typeof window.PK.syncCanvasPreviewWindowLayout === 'function'
...
function setWizardPreviewBackMode(mode) {
  if (!previewDoubleSided()) return;
  state.wizardDraft.preview = state.wizardDraft.preview || {};
  state.wizardDraft.preview.backMode = (mode === 'blank' || mode === 'reflect') ? mode : 'mirror';
  state.previewFlipped = true;
  pushWizardHistory();
  rerenderWizardPreviewPanel();
}
...
function syncWizardShapeEditorUi() {
  ...
  root.querySelectorAll('[data-shape-choice]').forEach(function(button){
    var selected = String(button.getAttribute('data-shape-choice') || '').trim() === activeType;
    button.classList.toggle('sel', selected);
  });
